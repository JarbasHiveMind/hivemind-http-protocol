...
class TestGetClient:
    ...
    def test_do_disconnect_removes_client_and_queue(self, master, caplog):
        proto = master.hm_protocol
        _clean_class_state()
        HiveMindHttpHandler.hm_protocol = proto
    ...
    def test_do_disconnect_is_idempotent(self, master, caplog):
        """The core's handle_client_disconnected ends by calling
        client.disconnect() again; that second call must be a no-op
        rather than recursing back into handle_client_disconnected."""
        proto = master.hm_protocol
        _clean_class_state()
        HiveMindHttpHandler.hm_protocol = proto
        ...
        with patch.object(proto.db, "get_client_by_api_key", return_value=user):
            h = HiveMindHttpHandler.__new__(HiveMindHttpHandler)
            client = h.get_client("agent", "disckey2", cache=True)
            with patch.object(
                proto, "handle_client_disconnected", wraps=proto.handle_client_disconnected
            ) as mock_disconnected:
                with caplog.at_level("DEBUG"):
                    client.disconnect()
                mock_disconnected.assert_called_once_with(client)
            assert not [r for r in caplog.records if r.levelname == "ERROR"]
