...
Poll for pending text messages from the hub.
...
Poll for pending binary messages from the hub (e.g. TTS audio).
...
The HTTP transport is stateful on the server side: the hub buffers outbound
